...
void multi_target_attack_analysis_strategy::analyze_impl(attack_analysis& aas,
	...
	for(size_t i=0;i<target_number;++i){
		...
		aas_res[i].leader_threat = (tile != 6);
		aas_res[i].uses_leader = false;
		
		aas_res[i].target_value = defend_it->cost();
		aas_res[i].target_value += (double(defend_it->experience())/
						 double(defend_it->max_experience()))*aas_res[i].target_value;
		aas_res[i].target_starting_damage = defend_it->max_hitpoints() - defend_it->hitpoints();
		...
		// making itself.
		aas_res[i].alternative_terrain_quality = 0.0;
		double cost_sum = 0.0;
		for(size_t j = 0; j != aas.movements.size(); ++j) {
			const unit_map::const_iterator att = units.find(aas.movements[j].first);
			const double cost = att->cost();
			cost_sum += cost;
			aas_res[i].alternative_terrain_quality += cost*ai_obj.best_defensive_position(aas.movements[j].first,dstsrc,srcdst,enemy_dstsrc).chance_to_hit;
		}
		aas_res[i].alternative_terrain_quality /= cost_sum*100;
		
		aas_res[i].avg_damage_inflicted = 0.0;
		aas_res[i].avg_damage_taken = 0.0;
		aas_res[i].resources_used = 0.0;
		aas_res[i].terrain_quality = 0.0;
		aas_res[i].avg_losses = 0.0;
		aas_res[i].chance_to_kill = 0.0;
		aas_res[i].is_visited = false;
	...
	assert(aas.movements.size() == aas.movements_target_ids.size());
	for (size_t i=0; i<aas.movements.size(); ++i) {
		// Log this movement
		LOG_AI_MT<<"The movement: from " <<aas.movements[i].first << " to " << aas.movements[i].second << std::endl;
		
		// Target id for this movement
		int tid = aas.movements_target_ids[i];
		const map_location& target = aas_res[tid].target;

		aas_res[tid].is_visited = true;
		const std::pair<map_location,map_location>& m = aas.movements[i];
		...
		if (up->can_recruit()) {
			aas_res[tid].uses_leader = true;
			// FIXME: suokko's r29531 omitted this line
			aas_res[tid].leader_threat = false;
		...
		double prob_fought = (1.0 - prob_dead_already[tid]);
		
		/** @todo 1.9 add combatant.prob_killed */
		double prob_killed = def.hp_dist[0] - prob_dead_already[tid];
		prob_dead_already[tid] = def.hp_dist[0];
		...
		aas_res[tid].resources_used += cost;
		aas_res[tid].avg_losses += cost * prob_died;
		
		// add half of cost for poisoned unit so it might get chance to heal
		aas_res[tid].avg_losses += cost * up->get_state(unit::STATE_POISONED) /2;
		
		if (!bc[tid]->get_defender_stats().is_poisoned) {
			aas_res[tid].avg_damage_inflicted += game_config::poison_amount * 2 * bc[tid]->get_defender_combatant().poisoned * (1 - prob_killed);
		}
		
		// Double reward to emphasize getting onto villages if they survive.
		if (on_village) {
			aas_res[tid].avg_damage_taken -= game_config::poison_amount*2 * prob_survived;
		}
		
		aas_res[tid].terrain_quality += (double(bc[tid]->get_defender_stats().chance_to_hit)/100.0)*cost * (on_village ? 0.5 : 1.0);
		...
		if (!up->advances_to().empty()) {
			...
			if (fight_xp >= xp_for_advance) {
				...
				aas_res[tid].avg_losses -= up->cost() * 0.25 *
			...
			if (bc[tid]->get_attacker_stats().plagues) {
				aas_res[tid].avg_losses -= prob_killed * up->cost();
			}
		}
		
		// If we didn't advance, we took this damage.
		aas_res[tid].avg_damage_taken += (up->hitpoints() - att.average_hp()) * (1.0 - advance_prob);
		...
		int kill_xp = game_config::kill_xp(fight_xp);
		def_avg_experience[tid] += fight_xp * (1.0 - att.hp_dist[0]) + kill_xp * att.hp_dist[0];
		if (i == 0) { // first movement
			first_chance_kill[tid] = def.hp_dist[0];
		}
	}
	
	for(size_t i=0;i<target_number;++i){
		if(!aas_res[i].is_visited) {
			// If not a target in this attack. 
			// do not compute values
			continue;
		}
		
		if (!defenders[i]->advances_to().empty() &&
			def_avg_experience[i] >= defenders[i]->max_experience() - defenders[i]->experience()) {
			// It's likely to advance: only if we can kill with first blow.
			aas_res[i].chance_to_kill = first_chance_kill[i];
			// Negative average damage (it will advance).
			aas_res[i].avg_damage_inflicted = defenders[i]->hitpoints() - defenders[i]->max_hitpoints();
		} else {
			aas_res[i].chance_to_kill = prev_def[i]->hp_dist[0];
			aas_res[i].avg_damage_inflicted = defenders[i]->hitpoints() - prev_def[i]->average_hp(map.gives_healing(defenders[i]->get_location()));
		}
		aas_res[i].terrain_quality /= aas_res[i].resources_used;
	}
	...
	int tid = aas.movements_target_ids[0];
	
	aas.target = aas_res[tid].target;
	aas.target_value = aas_res[tid].target_value;
	aas.avg_losses = aas_res[tid].avg_losses;
	aas.chance_to_kill = aas_res[tid].chance_to_kill;
	aas.avg_damage_inflicted = aas_res[tid].avg_damage_inflicted;
	aas.target_starting_damage = aas_res[tid].target_starting_damage;
	aas.avg_damage_taken = aas_res[tid].avg_damage_taken;
	aas.resources_used = aas_res[tid].resources_used;
	aas.terrain_quality = aas_res[tid].terrain_quality;
	aas.alternative_terrain_quality = aas_res[tid].alternative_terrain_quality;
	aas.vulnerability = aas_res[tid].vulnerability;
	aas.support = aas_res[tid].support;
	aas.leader_threat = aas_res[tid].leader_threat;
	aas.uses_leader = aas_res[tid].uses_leader;
	aas.is_surrounded = aas_res[tid].is_surrounded;
}
...
double multi_target_attack_analysis_strategy::rating_impl(const attack_analysis& aas, double aggression, const readonly_context& ai_obj) const{
	...
	for (size_t i=0; i<target_number; ++i) {
		...
		if(!aas_res[i].is_visited || aas_res[i].movements.empty()) {
			continue;
		}
	
		if(aas_res[i].leader_threat) {
			aggression = 1.0;
		}
		
		if(aas_res[i].uses_leader) {
			aggression = ai_obj.get_leader_aggression();
		}
		
		values[i] = aas_res[i].chance_to_kill*aas_res[i].target_value - aas_res[i].avg_losses*(1.0-aggression);
		
		if(aas_res[i].terrain_quality > aas_res[i].alternative_terrain_quality) {
	...
	#else
			const double exposure_mod = aas_res[i].uses_leader ? 2.0 : ai_obj.get_caution();
			const double exposure = exposure_mod*aas_res[i].resources_used*(aas_res[i].terrain_quality - aas_res[i].alternative_terrain_quality)*aas_res[i].vulnerability/std::max<double>(0.01,aas_res[i].support);
	#endif
			LOG_AI << "attack option has base value " << values[i] << " with exposure " << exposure << ": "
			<< aas_res[i].vulnerability << "/" << aas_res[i].support << " = " << (aas_res[i].vulnerability/std::max<double>(aas_res[i].support,0.1)) << "\n";
			values[i] -= exposure*(1.0-aggression);
		}
		
		// Prefer to attack already damaged targets.
		values[i] += ((aas_res[i].target_starting_damage/3 + aas_res[i].avg_damage_inflicted) - (1.0-aggression)*aas_res[i].avg_damage_taken)/10.0;
		...
		if(!aas_res[i].is_surrounded || (aas_res[i].support != 0 && aas_res[i].avg_damage_taken != 0))
		{
			...
			if(aas_res[i].vulnerability > 50.0 && aas_res[i].vulnerability > aas_res[i].support*2.0
			   && aas_res[i].chance_to_kill < 0.02 && aggression < 0.75
			   && !aas.attack_close(aas_res[i].target)) {
				return -1.0;
			}
		}
		
		if(!aas_res[i].leader_threat && aas_res[i].vulnerability*aas_res[i].terrain_quality > 0.0) {
			values[i] *= aas_res[i].support/(aas_res[i].vulnerability*aas_res[i].terrain_quality);
		}
		
		values[i] /= ((aas_res[i].resources_used/2) + (aas_res[i].resources_used/2)*aas_res[i].terrain_quality);
	...
	LOG_AI << "attack on " << aas_res[tid].target << ": attackers: " << aas_res[tid].movements.size()
	<< " value: " << values[tid] << " chance to kill: " << aas_res[tid].chance_to_kill
	<< " damage inflicted: " << aas_res[tid].avg_damage_inflicted
	<< " damage taken: " << aas_res[tid].avg_damage_taken
	<< " vulnerability: " << aas_res[tid].vulnerability
	<< " support: " << aas_res[tid].support
	<< " quality: " << aas_res[tid].terrain_quality
	<< " alternative quality: " << aas_res[tid].alternative_terrain_quality << "\n";
