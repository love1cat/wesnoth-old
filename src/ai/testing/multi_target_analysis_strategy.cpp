...
void multi_target_analysis_strategy::do_multiple_target_analysis(const analysis_inputs& aas_inp, 
	...
	if(target_count == target_number){
		...
		for(size_t i=0;i<cur_target_locs.size();++i){
			analysis.analysis_results[i].target = cur_target_locs[i];
		}
		...
		for(size_t i=0;i<cur_target_locs.size();++i){
			...
			get_adjacent_tiles(cur_target_locs[i], adj_array);
		...
		for(size_t i=0;i<adj_tiles.size();++i){
			for(size_t j=0;j<6;++j){
				used_locations[adj_tiles[i][j]] = false;
			}
		}
	...
	for(size_t i=target_count;i<=target_locs.size()-(target_number-target_count);++i){
		cur_target_locs.push_back(target_locs[i]);
...
void multi_target_analysis_strategy::do_attack_analysis(const analysis_inputs& aas_inp,
	...
	for(size_t i = 0; i != units.size(); ++i) {
		...
		for(size_t j = 0; j < adj_tiles.size(); ++j) {
			const std::vector<map_location>& adj_locs = adj_tiles[j];
			const map_location& target = target_locs[j];
			
			for(size_t k = 0;k < 6; ++k){
				const map_location& loc = adj_locs[k];
		...
		if(cur_position != -1) {
			const map_location& best_tile = adj_tiles[best_target_id][cur_position];
			units.erase(units.begin() + i);
			
			cur_analysis.analysis_results[best_target_id].movements.push_back(attack_analysis::movement_step(current_unit,best_tile));
			...
			cur_analysis.analysis_results[best_target_id].vulnerability += best_vulnerability;
			
			cur_analysis.analysis_results[best_target_id].support += best_support;
			
			cur_analysis.analysis_results[best_target_id].is_surrounded = is_surrounded;
			
			for(size_t j=0;j<cur_analysis.movements_target_ids.size();++j){
				LOG_AI<<cur_analysis.movements_target_ids[j]<<std::endl;
			}
			...
			cur_analysis.analysis_results[best_target_id].movements.pop_back();
